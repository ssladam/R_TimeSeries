...
# boxplot(full.X.comp$station_max_temp_c)
# boxplot(full.X.comp$reanalysis_max_air_temp_k)
# boxplot(full.X.comp$reanalysis_tdtr_k)

#full.X.comp %<>% select(-c(seas_min_temp, seas_max_temp, seas_avg_temp))
...
#check_ccf(iq.cases, iq.train$ndvi_sw, 'ndvi_nw')
...
check_ccf(sj.cases, sj.train$seas_max_temp, 'seas_max_temp') #include at 7
check_ccf(sj.cases, sj.train$seas_avg_temp, 'seas_avg_temp') #include at 9

full.sj %<>% mutate(lag10_ndvi_nw = lag(ndvi_nw, 10))
full.sj %<>% mutate(lag21_ndvi_se = lag(ndvi_se, 21))
full.sj %<>% mutate(lag11_ndvi_nw = lag(ndvi_nw, 11))
full.sj %<>% mutate(lag11_ndvi_se = lag(ndvi_ne, 11))
full.sj %<>% mutate(lag11_ndvi_se = lag(ndvi_se, 11))
full.sj %<>% mutate(lag11_ndvi_se = lag(ndvi_sw, 11))
...
full.sj %<>% mutate(lag7_seas_max_temp = lag(seas_max_temp, 7))
full.sj %<>% mutate(lag9_seas_avg_temp = lag(seas_avg_temp, 9))


full.iq %<>% mutate(lag10_ndvi_nw = lag(ndvi_nw, 10))
full.iq %<>% mutate(lag21_ndvi_se = lag(ndvi_se, 21))
full.iq %<>% mutate(lag11_ndvi_nw = lag(ndvi_nw, 11))
full.iq %<>% mutate(lag11_ndvi_se = lag(ndvi_ne, 11))
full.iq %<>% mutate(lag11_ndvi_se = lag(ndvi_se, 11))
full.iq %<>% mutate(lag11_ndvi_se = lag(ndvi_sw, 11))
...
full.iq %<>% mutate(lag7_seas_max_temp = lag(seas_max_temp, 7))
full.iq %<>% mutate(lag9_seas_avg_temp = lag(seas_avg_temp, 9))
...
main.xreg <- c('weekofyear','lag10_ndvi_nw','reanalysis_tdtr_k','lag10_ndvi_nw',
               ...
               'lag10_station_min_temp_c','lag8_seas_min_temp', 'lag7_seas_max_temp', 'lag9_seas_avg_temp',
               'case_lag1')
...
full_model <- get_best_model(train.X.sj, main.xreg,1)
validate_perf(full_model, train.X.sj %>% select(main.xreg), train.X.sj)

cust_model <- get_best_model(train.X.sj, cust.xreg,1)
validate_perf(cust_model, train.X.sj %>% select(cust.xreg), train.X.sj)
...
xreg.sj = test.X.sj %>% select(main.xreg)
...
validate_perf(best_model, train.X.iq %>% select(cust.xreg), train.X.iq)
